	...
	function loadInfo(){
		...
		xmlHttp.onreadystatechange=function(){
			if(xmlHttp.readyState==4 && xmlHttp.status==200){
				...
				var dataObj=eval("("+xmlHttp.responseText+")");//将返回的json串转为json对象
				alert(dataObj.name);
				alert(dataObj.age);
		...
		xmlHttp.onreadystatechange=function(){
			if(xmlHttp.readyState==4 && xmlHttp.status==200){
				...
				for(var i=0;i<dataObj.students.length;i++){
					...
					newTr=st.insertRow(); //向表格插入行
					newTd0=newTr.insertCell();//在刚才所插入的行中插入列
...
<div style="text-align: center;">
	<div><input type="button" onclick="loadInfo()" value="Ajax获取信息"/>&nbsp;&nbsp;
	姓名：<input type="text" id="name" name="name" />&nbsp;&nbsp;年龄：<input type="text" id="age" name="age" /></div>
